...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			subprob.x = new AttributeValue[subprob.l][];
	...
	public static svm_model svm_train(svm_problem prob, svm_parameter param)
	{
		   ...
		   param.svm_type == svm_parameter.NU_SVR)
		{
			...
			model.l = nSV;
			model.SV = new AttributeValue[nSV][];
		...
		else
		{
			...
			AttributeValue[][] x = new AttributeValue[l][];
			int i;
			for(i=0;i<l;i++)
				x[i] = prob.x[perm[i]];
			...
			for(i=0;i<nr_class;i++)
				for(int j=i+1;j<nr_class;j++)
				{
					...
					sub_prob.l = ci+cj;
					sub_prob.x = new AttributeValue[sub_prob.l][];
			...
			model.l = nnz;
			model.SV = new AttributeValue[nnz][];
	...
	public static double svm_predict_values(svm_model model, AttributeValue[] x, double[] dec_values)
	{
		int i;
	...
	public static double svm_predict(svm_model model, AttributeValue[] x)
	{
		int nr_class = model.nr_class;
		double[] dec_values;
		if(model.param.svm_type == svm_parameter.ONE_CLASS ||
	...
	public static double svm_predict_probability(svm_model model, AttributeValue[] x, double[] prob_estimates)
	{
	...
	public static svm_model svm_load_model(BufferedReader fp) throws IOException
	{
		...
		for(int i=0;i<l;i++)
		{
			...
			model.SV[i] = new AttributeValue[n];
			for(int j=0;j<n;j++)
			{
				model.SV[i][j] = new AttributeValue();
