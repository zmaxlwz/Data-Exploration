...
public class Region implements IShape
{
	...
	//
	@Override
	public Region intersectRegion(final IShape s) {
		if (s instanceof Region) 
			return intersectRegion((Region) s);

		throw new IllegalStateException("intersectRegion: Not implemented yet!");
	}
	...
	public Region intersectRegion(final Region r) {
		if(m_pLow.length != r.m_pLow.length)
			throw new IllegalArgumentException("intersectRegion: Shape has the wrong number of dimensions.");
		double[] lb = new double[m_pLow.length];
		double[] ub = new double[m_pLow.length];
		boolean valid = true;
		for (int cDim = 0; cDim < m_pLow.length; cDim++) {
			lb[cDim] = Math.max(m_pLow[cDim], r.m_pLow[cDim]);
			ub[cDim] = Math.min(m_pHigh[cDim], r.m_pHigh[cDim]);
			if(lb[cDim]>ub[cDim]) {
				valid = false;
				break;
			}
