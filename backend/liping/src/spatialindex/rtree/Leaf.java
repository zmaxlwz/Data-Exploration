...
{
	public Leaf(RTree pTree, long l_id)
	{
		super(pTree, l_id, 0, pTree.m_leafCapacity);
	}
	...
	protected Leaf findLeaf(Region mbr, long l_id, Stack pathBuffer)
	{
		...
		{
			if (l_m_pIdentifier[cChild] == l_id && mbr.equals(m_pMBR[cChild])) return this;
		}
	...
	protected Node[] split(byte[] pData, Region mbr, long l_id)
	{
		...
		for (cIndex = 0; cIndex < g1.size(); cIndex++)
		{
			...
			left.insertEntry(m_pData[i], m_pMBR[i], l_m_pIdentifier[i]);
		...
		for (cIndex = 0; cIndex < g2.size(); cIndex++)
		{
			...
			right.insertEntry(m_pData[i], m_pMBR[i], l_m_pIdentifier[i]);
	...
	protected void deleteData(long l_id, Stack pathBuffer)
	{
		int child;
		for (child = 0; child < m_children; child++)
		{
			if (l_m_pIdentifier[child] == l_id) break;
		}
		...
		while (! toReinsert.empty())
		{
			...
			for (int cChild = 0; cChild < n.m_children; cChild++)
			{
				...
				m_pTree.insertData_impl(n.m_pData[cChild],
																n.m_pMBR[cChild], n.l_m_pIdentifier[cChild],
