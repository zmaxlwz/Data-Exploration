...
{
	public RWLock m_rwLock;

	IStorageManager m_pStorageManager;

	public long l_m_rootID;
	long l_m_headerID;
	...
	public RTree(PropertySet ps, IStorageManager sm)
	{
		...
		l_m_rootID = IStorageManager.l_NewPage;
		l_m_headerID = IStorageManager.l_NewPage;
		m_treeVariant = SpatialIndex.RtreeVariantRstar;
		m_fillFactor = (Double)(ps.getProperty("FillFactor"));
		m_indexCapacity = (Integer)(ps.getProperty("IndexCapacity"));
		m_leafCapacity = (Integer)(ps.getProperty("LeafCapacity"));
		m_nearMinimumOverlapFactor = 32;
		m_splitDistributionFactor = 0.4f;
		m_reinsertFactor = 0.3f;
		m_dimension = (Integer)(ps.getProperty("Dimension"));
		...
		if (var != null)
		{
			if (! (var instanceof Long)) throw new IllegalArgumentException("Property IndexIdentifier must an Long");
			l_m_headerID = ((Long) var).longValue();//TODO: int or long?
			try
			{
				initOld(ps);
			}
		...
		else
		{
			...
			Long i = new Long(l_m_headerID);
			ps.setProperty("IndexIdentifier", i);//TODO: int or long?
		}
	}
	...
	public void insertData(final byte[] data, final IShape shape, long l_id)
	{
		...
		try
		{
			...
			insertData_impl(buffer, mbr, l_id);
	...
	public boolean deleteData(final IShape shape, long l_id)
	{
		...
		{
			Region mbr = shape.getMBR();
			return deleteData_impl(mbr, l_id);
		}
		finally
		{
			m_rwLock.write_unlock();
			...
			while (queue.size() != 0)
			{
				...
				if (first.m_pEntry instanceof Node)
				{
					...
					for (int cChild = 0; cChild < n.m_children; cChild++)
					{
						...
						if (n.m_level == 0)
						{
							e = new Data(n.m_pData[cChild], n.m_pMBR[cChild], n.l_m_pIdentifier[cChild]);
						}
						else
						{
							e = (IEntry) readNode(n.l_m_pIdentifier[cChild]);
						}
				...
				else
				{
					...
					//  furthest distance).
					if (l_count >= k && first.m_minDist > knearest) break;
	...
	public boolean isIndexValid()
	{
		...
		while (! st.empty())
		{
			...
			if (e.m_pNode.m_level != 0)
			{
				for (int cChild = 0; cChild < e.m_pNode.m_children; cChild++)
				{
					ValidateEntry tmpEntry = new ValidateEntry(e.m_pNode.m_pMBR[cChild], readNode(e.m_pNode.l_m_pIdentifier[cChild]));
	...
	private void storeHeader() throws IOException
	{
		...
		ds.flush();
		l_m_headerID = m_pStorageManager.storeByteArray(l_m_headerID, bs.toByteArray());
	}

	private void loadHeader() throws IOException
	{
		byte[] data = m_pStorageManager.loadByteArray(l_m_headerID);
		DataInputStream ds = new DataInputStream(new ByteArrayInputStream(data));

		l_m_rootID = ds.readLong();
	...
	protected void insertData_impl(byte[] pData, Region mbr, long l_id)
	{
		...
		l.insertData(pData, mbr, l_id, pathBuffer, overflowTable);

		m_stats.m_data++;
	}

	protected void insertData_impl(byte[] pData, Region mbr, long l_id, int level, boolean[] overflowTable)
	{
		assert mbr.getDimension() == m_dimension;
		...
		n.insertData(pData, mbr, l_id, pathBuffer, overflowTable);
	}

	protected boolean deleteData_impl(final Region mbr, long l_id)
	{
		assert mbr.getDimension() == m_dimension;
		...
		Node root = readNode(l_m_rootID);
		Leaf l = root.findLeaf(mbr, l_id, pathBuffer);

		if (l != null)
		{
			l.deleteData(l_id, pathBuffer);
			m_stats.m_data--;
			bRet = true;
		}
	...
	protected long writeNode(Node n) throws IllegalStateException
	{
		byte[] buffer = null;
		...
		long l_page;
		if (n.l_m_identifier < 0) l_page = IStorageManager.l_NewPage;
		else l_page = n.l_m_identifier;

		try
		{
			l_page = m_pStorageManager.storeByteArray(l_page, buffer);
		}
		...
		if (n.l_m_identifier < 0)
		{
			n.l_m_identifier = l_page;
			m_stats.m_nodes++;
	...
	public Node readNode(long l_id)
	{
		byte[] buffer;
		...
		try
		{
			buffer = m_pStorageManager.loadByteArray(l_id);
	...
	protected void deleteNode(Node n)
	{
		try
		{
			m_pStorageManager.deleteByteArray(n.l_m_identifier);
		}
		catch (InvalidPageException e)
		{
			System.err.println(e);
			...
			while (! st.empty())
			{
				...
				if (n.m_level == 0)
				{
					...
					for (int cChild = 0; cChild < n.m_children; cChild++)
					{
						...
						if (b)
						{
							Data data = new Data(n.m_pData[cChild], n.m_pMBR[cChild], n.l_m_pIdentifier[cChild]);
							v.visitData(data);
							m_stats.m_queryResults++;
						}
					}
				...
				else
				{
					...
					for (int cChild = 0; cChild < n.m_children; cChild++)
					{
						...
						{
							st.push(readNode(n.l_m_pIdentifier[cChild]));
						}
					}
				}
			}
	...
	class Data implements IData
	{
		...
		Data(byte[] pData, Region mbr, long l_id) { m_id = l_id; m_shape = mbr; m_pData = pData; }

		public long getIdentifier() { return m_id; }
