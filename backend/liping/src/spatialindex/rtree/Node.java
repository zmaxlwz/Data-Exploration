...
public abstract class Node implements INode
{
	...
	protected long l_m_identifier = -1;
	...
	public long[] l_m_pIdentifier = null;
		// The corresponding data identifiers.

	protected int[] m_pDataLength = null;

	long l_m_totalDataLength = 0;

	public Integer[] m_pIdentifier;
	...
	protected abstract Leaf findLeaf(Region mbr, long l_id, Stack pathBuffer);
	protected abstract Node[] split(byte[] pData, Region mbr, long l_id);
	...
	public byte [] getChildrenData(int index) throws IndexOutOfBoundsException {
		if (index < 0 || index >= m_children) throw new IndexOutOfBoundsException("" + index);
		return m_pData[index];
	}

	public long getChildIdentifier(int index) throws IndexOutOfBoundsException
	{
		if (index < 0 || index >= m_children) throw new IndexOutOfBoundsException("" + index);

		return l_m_pIdentifier[index];
	}
	...
	protected Node(RTree pTree, long id, int level, int capacity)
	{
		m_pTree = pTree;
		m_level = level;
		l_m_identifier = id;
		...
		l_m_pIdentifier = new long[m_capacity + 1];
	}

	protected void insertEntry(byte[] pData, Region mbr, long id) throws IllegalStateException
	{
		...
		l_m_pIdentifier[m_children] = id;

		l_m_totalDataLength += m_pDataLength[m_children];
		m_children++;
	...
	protected void deleteEntry(int index) throws IndexOutOfBoundsException
	{
		...
		l_m_totalDataLength -= m_pDataLength[index];
		m_pData[index] = null;
		...
		if (m_children > 1 && index != m_children - 1)
		{
			...
			l_m_pIdentifier[index] = l_m_pIdentifier[m_children - 1];
		}
	...
	protected boolean insertData(byte[] pData, Region mbr, long l_id, Stack <Long> pathBuffer, boolean[] overflowTable)
	{
		if (m_children < m_capacity)
		{
			...
			insertEntry(pData, mbr, l_id);
			m_pTree.writeNode(this);

			if (! b && ! pathBuffer.empty())
			{
				long l_cParent = pathBuffer.pop().longValue();
				Index p = (Index) m_pTree.readNode(l_cParent);
		...
		else if (m_pTree.m_treeVariant == SpatialIndex.RtreeVariantRstar && ! pathBuffer.empty() && overflowTable[m_level] == false)
		{
			...
			reinsertData(pData, mbr, l_id, vReinsert, vKeep);
			...
			for (cIndex = 0; cIndex < lReinsert; cIndex++)
			{
				...
				l_reinsertid[cIndex] = l_m_pIdentifier[i];
			}
			...
			for (cIndex = 0; cIndex < lKeep; cIndex++)
			{
				...
				l_keepid[cIndex] = l_m_pIdentifier[i];
			}

			m_pDataLength = keeplen;
			m_pData = keepdata;
			m_pMBR = keepmbr;
			l_m_pIdentifier = l_keepid;
			m_children = lKeep;
			l_m_totalDataLength = 0;
			for (int cChild = 0; cChild < m_children; cChild++) l_m_totalDataLength += m_pDataLength[cChild];
		...
		{
			Node[] nodes = split(pData, mbr, l_id);
			Node n = nodes[0];
			Node nn = nodes[1];
			...
			if (pathBuffer.empty())
			{
				...
				Index r = new Index(m_pTree, m_pTree.l_m_rootID, m_level + 1);

				r.insertEntry(null, (Region) n.m_nodeMBR.clone(), n.l_m_identifier);
				r.insertEntry(null, (Region) nn.m_nodeMBR.clone(), nn.l_m_identifier);
			...
			{
				n.l_m_identifier = l_m_identifier;
				nn.l_m_identifier = -1;

				m_pTree.writeNode(n);
				m_pTree.writeNode(nn);

				long cParent = pathBuffer.pop().longValue();
	...
	protected void reinsertData(byte[] pData, Region mbr, long l_id, ArrayList reinsert, ArrayList keep)
	{
		...
		m_pData[m_children] = pData;
		m_pMBR[m_children] = mbr;
		l_m_pIdentifier[m_children] = l_id;
	...
	protected void rtreeSplit(byte[] pData, Region mbr, long l_id, ArrayList group1, ArrayList group2)
	{
		int cChild;
		...
		l_m_pIdentifier[m_capacity] = l_id;
	...
	protected void rstarSplit(byte[] pData, Region mbr, long l_id, ArrayList group1, ArrayList group2)
	{
	...
	protected void condenseTree(Stack toReinsert, Stack pathBuffer)
	{
		...
		if (pathBuffer.empty())
		{
			...
			{
				Node n = m_pTree.readNode(l_m_pIdentifier[0]);
				m_pTree.deleteNode(n);
				n.l_m_identifier = m_pTree.l_m_rootID;
				m_pTree.writeNode(n);
		...
		{
			long cParent = ((Long)pathBuffer.pop()).longValue();
			Index p = (Index) m_pTree.readNode(cParent);
			...
			{
				if (p.l_m_pIdentifier[child] == l_m_identifier) break;
			}
	...
	protected void load(byte[] data) throws IOException
	{
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			...
			l_m_pIdentifier[cChild] = ds.readLong();

			m_pDataLength[cChild] = ds.readInt();
			if (m_pDataLength[cChild] > 0)
			{
				l_m_totalDataLength += m_pDataLength[cChild];
	...
	protected byte[] store() throws IOException
	{
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			...
			ds.writeLong(l_m_pIdentifier[cChild]);
