...
{
	public Index(RTree pTree, long l_id, int level)
	{
		super(pTree, l_id, level, pTree.m_indexCapacity);
	}
	...
	protected Node chooseSubtree(Region mbr, int level, Stack pathBuffer)
	{
		...
		pathBuffer.push(new Long(l_m_identifier));
		...
		Node n = m_pTree.readNode(l_m_pIdentifier[child]);
		Node ret = n.chooseSubtree(mbr, level, pathBuffer);

		return ret;
	}

	protected Leaf findLeaf(Region mbr, long l_id, Stack pathBuffer)
	{
		pathBuffer.push(new Long(l_m_identifier));
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			...
			{
				Node n = m_pTree.readNode(l_m_pIdentifier[cChild]);
				Leaf l = n.findLeaf(mbr, l_id, pathBuffer);
				if (l != null) return l;
			}
		}
	...
	protected Node[] split(byte[] pData, Region mbr, long l_id)
	{
		...
		switch (m_pTree.m_treeVariant)
		{
			case SpatialIndex.RtreeVariantLinear:
			case SpatialIndex.RtreeVariantQuadratic:
				rtreeSplit(pData, mbr, l_id, g1, g2);
				break;
			case SpatialIndex.RtreeVariantRstar:
				rstarSplit(pData, mbr, l_id, g1, g2);
				break;
			default:
				throw new IllegalStateException("Unknown RTree variant.");
		}

		Node left = new Index(m_pTree, l_m_identifier, m_level);
		...
		for (cIndex = 0; cIndex < g1.size(); cIndex++)
		{
			...
			left.insertEntry(null, m_pMBR[i], l_m_pIdentifier[i]);
		}

		for (cIndex = 0; cIndex < g2.size(); cIndex++)
		{
			int i = ((Integer) g2.get(cIndex)).intValue();
			right.insertEntry(null, m_pMBR[i], l_m_pIdentifier[i]);
		}
	...
	protected void adjustTree(Node n, Stack pathBuffer)
	{
		...
		{
			if (l_m_pIdentifier[child] == n.l_m_identifier) break;
		}
	...
	protected void adjustTree(Node n1, Node n2, Stack pathBuffer, boolean[] overflowTable)
	{
		...
		for (child = 0; child < m_children; child++)
		{
			if (l_m_pIdentifier[child] == n1.l_m_identifier) break;
		}
		...
		boolean adjusted = insertData(null, (Region) n2.m_nodeMBR.clone(), n2.l_m_identifier, pathBuffer, overflowTable);
