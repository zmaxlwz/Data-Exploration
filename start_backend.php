...
if($_SESSION["scenario"] == "Scenario 5" && $_SESSION["target_query_selected"] == "query 1"){
	//start liping's backend for ellipses predefined query
	chdir('./backend/liping');
	exec('./run_ellipse.sh >>/dev/null 2>>/dev/null &');
}
elseif($_SESSION["scenario"] == "Scenario 5" && $_SESSION["target_query_selected"] == "query 2"){
	//start liping's backend for rectangle predefined query
	chdir('./backend/liping');
	exec('./run_rectangle.sh >>/dev/null 2>>/dev/null &');
}
elseif($_SESSION["scenario"] == "Scenario 6"){
